...
public class TamperWarningScreen extends Screen {
    ...
    public TamperWarningScreen(Screen parent) {
        super(Component.translatable("opsec.tamper.title"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        y += textLineHeight;
        addCenteredStringWidget(Component.translatable("opsec.tamper.warning"), centerX, y);
        y += textLineHeight;
        addCenteredStringWidget(Component.translatable("opsec.tamper.mismatch"), centerX, y);
        y += textLineHeight;
        addCenteredStringWidget(Component.translatable("opsec.tamper.malicious"), centerX, y);
        y += textLineHeight + textGap;
        addCenteredStringWidget(Component.translatable("opsec.tamper.compromised"), centerX, y);
        y += textLineHeight;
        addCenteredStringWidget(Component.translatable("opsec.tamper.action"), centerX, y);
        y += textLineHeight + hashGap;
        addCenteredStringWidget(Component.translatable("opsec.tamper.expected", truncate(JarIntegrityChecker.getExpectedDigest(), 16)), centerX, y);
        y += textLineHeight;
        addCenteredStringWidget(Component.translatable("opsec.tamper.actual", truncate(JarIntegrityChecker.getActualDigest(), 16)), centerX, y);
        y += textLineHeight + textToButtonGap;

        // Buttons stacked vertically, centered horizontally
        this.addRenderableWidget(Button.builder(Component.translatable("opsec.tamper.download"), button -> {
            Util.getPlatform().openUri(UpdateChecker.getReleaseUrl());
            this.onClose();
        }).bounds(centerX - buttonWidth / 2, y, buttonWidth, buttonHeight).build());

        this.addRenderableWidget(Button.builder(Component.translatable("opsec.tamper.dismiss_permanent"), button -> {
        ...
        this.addRenderableWidget(Button.builder(Component.translatable("opsec.tamper.dismiss"), button -> {
            this.onClose();
